    ...
    class ProtoConfig : public RCCopyable<thread_unsafe_refcount>
    {
      public:
        ...
        std::string peer_info_string(bool supports_epoch_data) const
        {
            ...
            std::string reported_platform = platform_name();
            if (extra_peer_info)
            {
                for (const auto &peer_info : *extra_peer_info)
                {
                    if (peer_info.key == "IV_PLAT")
                        reported_platform = peer_info.value;
                }
            }
