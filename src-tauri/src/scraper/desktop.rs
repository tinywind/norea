//! Desktop implementation of the scraper commands; see the parent module docs.
...
use tauri::{webview::PageLoadEvent, LogicalPosition, LogicalSize, Rect, Webview, WebviewBuilder};
...
use super::scripts::{
    build_webview_fetch_cancel_script, build_webview_fetch_cleanup_script,
    build_webview_fetch_poll_script, build_webview_fetch_start_script, SCRAPER_INIT_SCRIPT,
};
...
const BROWSER_CHALLENGE_DETECTOR_SCRIPT: &str =
    include_str!("../../../src/lib/plugins/browser-challenge-detector.js");

async fn document_has_browser_challenge(scraper: &ScraperWebview) -> bool {
    // The desktop retry loop and fallback envelope only handle Cloudflare;
    // captcha pages are reported by the in-page snapshot script instead.
    let kind =
        eval_json::<Option<String>>(scraper, format!("({BROWSER_CHALLENGE_DETECTOR_SCRIPT})()"))
            .await;
    matches!(kind, Ok(Some(ref kind)) if kind == "cloudflare")
}
...
pub async fn scraper_take_captured_resource(
    ...
    let resource =
        take_captured_resource_for_source(&state, &queue, &source_id, user_agent.as_deref(), &url);
...
mod tests {
    ...
    fn visible_user_navigation_replaces_previous_captured_resources() {
        ...
        *state.visible_key.lock().expect("scraper visible_key mutex") =
        ...
        let capture_id =
            begin_visible_user_navigation_resource_capture(&state, IMMEDIATE_EXECUTOR, "source-a")
    ...
    fn registered_navigation_keeps_its_existing_capture_session() {
        ...
        *state.visible_key.lock().expect("scraper visible_key mutex") =
        ...
        let _completion =
            register_navigation(&state, IMMEDIATE_EXECUTOR, 1, "https://source.test/page");
